...
# to process energy cycle vars for the selected run
#WRF_HYDRO_DIR=/g/data/w28/ad9701/WRF-Hydro/SEA/runs_wCalibPars_4km/wrf_hydro_nwm_public-5.2.0_domain4_4km_12g_defSoil_defPar/trunk/NDHMS/Run/
#WRF_HYDRO_DIR=/g/data/w28/ad9701/WRF-Hydro/SEA/runs_wCalibPars_4km/wrf_hydro_nwm_public-5.2.0_domain4_4km_LSMonly_defSoil_defPar/trunk/NDHMS/Run/
...
### **********************
#Final runs using the calibrated parameters & no baseflow
...
#WRF_HYDRO_DIR=/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_4km/wrf_hydro_nwm_public-5.2.0_domain4_4km_monPnTCorr_LSMonly_tern_defPar/trunk/NDHMS/Run/
...
#** TO DO: the 100m RT run is in progress

#*** 10km, 4km, 1km, 167m RT
...
####**************************************
#### *** The final runs used in the paper. noGW, default soils, default soil parameters
# Added a loop for WRF_HYDRO_DIR to process the 4-km and 10-km; the 1-km runs require a lot more memory to use the directory ../concat_var_singlefile_1km for that

myArray=("/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_4km/wrf_hydro_nwm_public-5.2.0_domain4_4km_monPnTCorr_LSMonly_defSoil_defPar/trunk/NDHMS/Run/" \
"/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_10km/wrf_hydro_nwm_public-5.2.0_domain4_10km_monPnTCorr_LSMonly_defSoil_defPar/trunk/NDHMS/Run/" \
"/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_4km_100mRT/wrf_hydro_nwm_public-5.2.0_domain4_4km_t500_RTCHon_4g_monPnTCorr_defSoil_defPar/trunk/NDHMS/Run/" \
"/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_10km_100mRT/wrf_hydro_nwm_public-5.2.0_trial2/trunk/NDHMS/Run/" \
"/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_4km/wrf_hydro_nwm_public-5.2.0_domain4_4km_t80_RTCHon_12g_monPnTCorr_defSoil_defPar/trunk/NDHMS/Run/" \
"/g/data/w97/ad9701/WRF-Hydro/SEA/noGW_runs/runs_wCalibPars_10km/wrf_hydro_nwm_public-5.2.0_domain4_10km_monPnTCorr_t80_RTCHon_defSoil_defPar/trunk/NDHMS/Run/")
...
#for varname in inflow outflow depth   #*** baseflow vars
#for varname in SOIL_M
#for varname in streamflow
#for varname in QSTRMVOLRT zwattablrt sfcheadsubrt

for varname in T2M #T2MB T2MV
# side note: T2M is calculated from T2MB and T2MV using calculate_T2M_from_T2MV_and_T2MB.ipynb
...
#*****
#ran out of walltime
#for varname in SWFORC WA

# radiation variables for both LSM and RT runs
#for varname in SWFORC LWFORC SAV FSA FIRA IRC IRG GRDFLX
#for varname in ALBEDO

# variables to look at runoff processes
#for varname in SOILSAT_TOP CANWAT ACSNOM FSNO QRAIN QSNOW
...
if [ "$varname" = QSTRMVOLRT ] || [ "$varname" = zwattablrt ] || [ "$varname" = sfcheadsubrt ] || [ "$varname" = infxsrt ] || [ "$varname" = SFCRNOFF ] || [ "$varname" = UGDRNOFF ] || [ "$varname" = ACCPRCP ] || [ "$varname" = ACCET ] || [ "$varname" = T2MB ] || [ "$varname" = T2MV ] || [ "$varname" = T2M ] ; then
calc_daily=false
else
calc_daily=true
fi
...
cp job_extract_template.pbs extract_script.pbs
sed -i 's|WRF_HYDRO_DIR|WRF_HYDRO_DIR='$WRF_HYDRO_DIR'|g' extract_script.pbs
sed -i 's|varname|varname='$varname'|g' extract_script.pbs
sed -i 's|file_suffix|file_suffix='$file_suffix'|g' extract_script.pbs
...
cp job_calc_daily_template_highmem.pbs daily_script.pbs
else
cp job_calc_daily_template.pbs daily_script.pbs
fi
...
cp job_calc_dailymax_template.pbs dailymax_script.pbs
...
if [ "$concat_hrlydata" = true ] ; then
job3=$(qsub -W depend=afterok:$job1 dailymax_script.pbs)
else
job3=$(qsub dailymax_script.pbs)
fi
echo $job3
fi
